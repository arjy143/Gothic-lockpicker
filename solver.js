/*
 * BFS solver for the gate puzzle. Port of puzzle_solver.py.
 *
 * States are packed into one integer (base-7 digits, value + 3 per slot) so the
 * frontier and the parent table can live in typed arrays.
 */
(function (root) {
  'use strict';
  ...
  // rules[i][j] = +1 (same op on j) or -1 (opposite op on j). Accepts an array
  // or an object keyed by index. Self-references and bad targets are dropped.
  ...
  // Returns { status, steps, explored } where status is 'solved', 'unsolvable'
  // or 'limit', and steps is [{ index, delta, state }].
    ...
    while (head < tail) {
      ...
      for (var m = 0; m < moves.length; m++) {
        ...
        for (var s = 0; s < mv.slots.length; s++) {
          ...
          if (next < 0 || next >= RANGE) { legal = false; break; }
        }
        if (!legal) continue;

        var nk = key + mv.keyDelta;
        if (parentKey[nk] !== -1) continue;
